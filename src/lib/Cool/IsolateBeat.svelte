<script lang="ts">
	...
	const tabsSvelte4 = [
		...
		animation-name: cube-rotate-middle;
		animation-iteration-count: infinite;
		animation-direction: alternate;
		animation-timing-function: ease-in-out;
	}

	@keyframes cube-rotate-middle {
		from {
			transform: perspective(200px) rotate3d(0.4, 0.03, 0.1, 0deg);
		}

		25% {
			transform: perspective(400px) rotate3d(0.4, 0.03, 0.1, 5deg);
		}

		50% {
			transform: perspective(100px) rotate3d(0.4, 0.03, 0.1, 0deg);
		}
		to {
			transform: perspective(300px) rotate3d(0.4, 0.03, 0.1, -5deg);
		}
	}`
		}
	];
...
<CodeBlox>
    ...
    <div class="w-64 h-24 my-4 bg-orange-400/60 mx-auto isolateMiddle">
happening??
    </div>
    </div>
